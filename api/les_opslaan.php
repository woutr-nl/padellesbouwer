...
try {
    // Alle bestaande items verwijderen
    $les->deleteAllItems($les_id);
    
    // Nieuwe items toevoegen
    $success = true;
    foreach ($items as $item) {
        if (!isset($item['type']) || !isset($item['x']) || !isset($item['y'])) {
            continue;
        }
        ...
        if (!$les->addItem($les_id, $itemData)) {
            $success = false;
            break;
        }
    }
    ...
    if ($success) {
        sendSuccessResponse(['message' => 'Les succesvol opgeslagen']);
    } else {
        sendErrorResponse('Fout bij het opslaan van de les');
    }
    
} catch (Exception $e) {
    sendErrorResponse('Fout bij het opslaan: ' . $e->getMessage());
}
?> 